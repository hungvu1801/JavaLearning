...
public class NumberPalindrome {
    ...
    public static int reverseNumber (int number) {
        ...
        do {
            ...
            reverseNum = reverseNum * 10 + remained;
            number /= 10;
            if (number >= 1) {
                isNext = true;
            } else isNext = false;
        } while (isNext) ;
        return reverseNum * isPos;
    }
    ...
    public static boolean isPalindrome2 (int number) {
        int originNumber = number, reverseNum;
        int isPos = 1;
        if (number < 0) {
            isPos = -1;
        }
        ...
        if (number / 10 < 1) {
            ...
            reverseNum = reverseNum * isPos;
        }  
        if (reverseNum == originNumber) {
            return true;
        } else return false;
    }
}